...
module('Integration | Component | star-rating', function (hooks) {
  ...
  test('Renders the full and empty stars correctly', async function (assert) {
    ...
    assert
      ...
      .exists({ count: 1 }, 'The right amound of empty stars is rendered');
    ...
    assert
      ...
      .exists(
        { count: 2 },
        'The right amound of full stars is rendered after changing rating'
      );
    assert
      .dom('[data-test-rr="empty-star"]')
      .exists(
        { count: 3 },
        'The right amound of empty stars is rendered after changing rating'
      );
  });
